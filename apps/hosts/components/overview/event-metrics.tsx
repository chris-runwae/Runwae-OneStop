  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-surface sm:rounded-2xl">
        ...
        {showPeriodSelector && onPeriodChange && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="flex cursor-pointer items-center gap-1 rounded bg-muted px-2 py-1.5 text-xs font-medium tracking-tight text-body transition-colors hover:bg-muted/80 focus:outline-none focus:ring-2 focus:ring-ring"
              >
                {periodLabel}
      ...
      <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4">
        <p className="font-display text-2xl font-semibold leading-tight text-heading sm:text-3xl sm:leading-10">
          {value}
        </p>
        {change && (
