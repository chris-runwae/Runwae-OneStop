...
type SavedItemsSectionProps = {
  ...
  openSaveToItinerarySheet: (item: SavedItem) => void;
};
const SavedItemsSection = ({
  tripId,
  savedItems,
  handleRemoveSavedItem,
  openSaveToItinerarySheet,
    ...
    <>
      <FlashList
        ...
        renderItem={({ item, index }: { item: SavedItem; index: number }) => (
          <Link
            href={`/trips/hotel/${item.id}` as RelativePathString}
            key={item.id}
            asChild>
            <Link.Trigger>
              <View
                ...
                <TripItemCards
                  item={item}
                  key={item.id}
                  onPress={() => openSaveToItinerarySheet(item)}
                />
              </View>
            </Link.Trigger>
            <Link.Menu>
